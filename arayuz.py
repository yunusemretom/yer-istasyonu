...
class QPrimaryFlightDisplay(WidgetClass):
    ...
    def compute_horizon(self, p, b):
        ...
        # Pitch etkisini horizon'a yansıtmak için y-offset ekliyoruz
        pitch_offset = p * 2  # pitch etkisini artırmak için 2 ile çarpıyoruz
        
        if b != pi / 2 and b != -pi / 2:
            x1 = 0
            y1 = h / 2 - w / 2 * tan(b) + p / cos(b) + pitch_offset
            x2 = w
            y2 = h / 2 + w / 2 * tan(b) + p / cos(b) + pitch_offset
        elif b == pi / 2:
            x1 = 0
            y1 = -1
            x2 = w
